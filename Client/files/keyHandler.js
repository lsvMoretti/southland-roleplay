...
alt.on('ChatStatus', (status) => {
    IsChatOpen = status;
});
alt.onServer('setPlayerSpawned', (toggle) => {
    IsSpawned = toggle;
});
var sirenMute = false;
alt.everyTick(() => {
...
alt.on('keyup', (key) => {
    ...
    if (key === 0x51) {
        var isDriver = native.getPedInVehicleSeat(alt.Player.local.vehicle.scriptID, -1, 0) == alt.Player.local.scriptID;
        var isFrontPass = native.getPedInVehicleSeat(alt.Player.local.vehicle.scriptID, 0, 0) == alt.Player.local.scriptID;
    ...
    if (alt.Player.local.vehicle != undefined) {
        if (leftCtrlDown) {
            var pVeh = alt.Player.local.vehicle;
            var pVehId = pVeh.scriptID;
            var isDriver = native.getPedInVehicleSeat(alt.Player.local.vehicle.scriptID, -1, 0) == alt.Player.local.scriptID;
            var isFrontPass = native.getPedInVehicleSeat(alt.Player.local.vehicle.scriptID, 0, 0) == alt.Player.local.scriptID;
            if (key === 0x31) {
...
function enterVehicleAsPassenger() {
    ...
    for (let i = 0; i < numberSeats; i++) {
        let seatFree = native.isVehicleSeatFree(vehicle, i, false);
        if (!seatFree) {
            seatCount++;
        }
    }
    alt.log(`Number of seats taken: ${seatCount}.`);
    if (seatCount == numberSeats)
        return;
...
alt.onServer('OnlinePlayerList', (json) => {
    if (onlinePlayerWindow === undefined)
        return;

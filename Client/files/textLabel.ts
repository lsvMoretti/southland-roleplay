...
import * as playerHandler from 'files/player/PlayerHandler';
...
function createTextLabel(textLabelJson:string) {
...
function displayTextLabel(textLabel:any) {
    //var textLabel = JSON.parse(textLabelJson);

    let playerPos = alt.Player.local.pos;
    var range = textLabel.Range;
    
    var dimension:number = alt.Player.local.getSyncedMeta("PlayerDimension");

    if (dimension != textLabel.Dimension) return;
...
function deleteTextLabel(textLabelJson:string) {

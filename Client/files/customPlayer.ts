...
function setPlayerClothes(slot: number, drawable: number, texture: number) {
    native.setPedComponentVariation(alt.Player.local.scriptID, slot, drawable, texture, 0);
}

function setPlayerAccessory(slot: number, drawable: number, texture: number) {
    if (drawable === -1) {
...
function loadCustomPlayer(customCharacterJson: string, clothesJson: string, accessoryJson: string) {
...
function LoadNPCPreview(pos: number, customCharacterJson: string, clothesJson: string, accessoryJson: string, torso: number) {
...
function setCustomCharacter(ped: number, customCharacterJson: string, clothesJson: string, accessoryJson: string) {
    var customCharacter = JSON.parse(customCharacterJson);

    if (customCharacter === null || customCharacter === undefined) return;

...
function toggleSitting() {
    ...
    sitableObjects.forEach(object => {
        var obj = native.getClosestObjectOfType(lastPos.x, lastPos.y, lastPos.z, 3.0, native.getHashKey(object.propName), false, true, true);
    ...
    var distance = closest.dist;
    var object = closest.obj;
...
function removeEntityCollision(objectName, objectPos) {
...
function setEntityCollision(objectName, objectPos) {
    var object = native.getClosestObjectOfType(objectPos.x, objectPos.y, objectPos.z, 0.5, alt.hash(objectName), false, true, true);
    if (object == undefined)
        return;
    alt.log('Setting Collision for: ' + object);
...
function playerPositionFree() {
    ...
    var newPos = new alt.Vector3(entityPos.x, entityPos.y, entityPos.z + sittableProp.verticalOffset);
    lastPos = alt.Player.local.pos;
...
class propInfo {
    constructor(objectHash, distance, objectName) {
        this.obj = objectHash;
        this.dist = distance;
        this.objName = objectName;
    }
}
var propInfoList = new Array();
class SitableProp {
    constructor(propName, scenario, verticalOffset, forwardOffset, leftOffset) {

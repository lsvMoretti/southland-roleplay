...
//alt.setMsPerGameMinute(6000000000);

alt.onServer('OnTimeUpdate', () => {
    alt.setMsPerGameMinute(30000);
});

function freezePlayer(state: boolean) {
...
alt.onServer('unloadIpl', (iplString: string) => {
    native.removeIpl(iplString);
});

alt.onServer('loadProp', (propName: string) => {
    alt.setTimeout(() => {
        //var interiorId = native.getInteriorFromEntity(alt.Player.local.scriptID);
...
alt.onServer('unloadProp', (propName: string) => {
    alt.setTimeout(() => {
        //var interiorId = native.getInteriorFromEntity(alt.Player.local.scriptID);
...
alt.onServer('SetInteriorColor', (propName: string, colorId: number) => {
    let player = alt.Player.local;
    let interiorId = native.getInteriorAtCoords(player.pos.x, player.pos.y, player.pos.z);
...
function CharacterLoaded(state: boolean) {
...
alt.onServer('setWaypoint', (pos: alt.Vector3) => {
...
alt.onServer('fetchPlayerStreetArea', (eventName: string) => {
...
function switchOutPlayer(timeout: number) {
    var player = alt.Player.local.scriptID;

    //    native.switchOutPlayer(player, 0, 1);
    native.doScreenFadeOut(1000);
...
var specCam: number = undefined;
var specPlayer: any = undefined;
var specInterval: number = undefined;

alt.onServer('ToggleSpectate', (state: boolean, player: alt.Entity) => {
    if (state) {
...
alt.onServer('ToggleTransparency',
    (state: boolean) => {
...
alt.onServer('SetWalkStyle', (style: number) => {
...
alt.onServer('SendPlayerDimension', (dimension: number) => {
